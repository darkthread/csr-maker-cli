...
if ($keyIsEncrypted) {
    & $opensslPath pkcs12 -export -out $pfxPath -inkey $PrivKeyPath -in $pemPath -passout pass:$plainPasswd
    Remove-Item $PrivKeyPath -ErrorAction SilentlyContinue
}
else {
    & $opensslPath pkcs12 -export -out $pfxPath -inkey $PrivKeyPath -in $pemPath
}
